...
class DDPMScheduler(BaseScheduler):
    ...
    def step(self, x_t: torch.Tensor, t: int, eps_theta: torch.Tensor):
        ...
        eps_factor = (1 - self._get_teeth(self.alphas, t)) / (
            1 - self._get_teeth(self.alphas_cumprod, t)
        ).sqrt()
        coef = 1 / self._get_teeth(self.alphas, t).sqrt()
        noise_coef = self._get_teeth(self.sigmas, t)
        
        noise = torch.randn_like(x_t) if t > 0 else 0.
        sample_prev = coef * (x_t - eps_factor * eps_theta) + noise_coef * noise
        #######################
